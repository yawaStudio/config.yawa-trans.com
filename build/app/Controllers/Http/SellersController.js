...
class SellersController {
    ...
    async store({ request, response }) {
        ...
        try {
            const save = await Prisma_1.prisma.seller.create({
                data: {
    ...
    async edit({ params, request, response }) {
        ...
        const save = await Prisma_1.prisma.seller.update({
            where: { id: id },
            data: {
                name: data.name,
                phone: data.phone,
    ...
    async view({ params, view }) {
        const id = params.id;
        const item = await Prisma_1.prisma.seller.findFirst({
            where: {
                id: id
            },
            include: {
    ...
    async destroy({ params, response }) {
        const id = params.id;
        await Prisma_1.prisma.seller.delete({
            where: {
                id
            },
        });
